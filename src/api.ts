...
import { getToken, storeUserId } from "./store"
...
instance.interceptors.request.use(
  (config) => {
    const conf = getToken().then((token) => {
      if (token) {
...
export function getUserMe() {
  return instance
    .get(`/users/me`)
    .then((res) => res.data.id).then((id)=>storeUserId(id))
    .catch(() => null)
}

export function getExerciseInfo(id: number) {
  return instance
    .get(`/exercises/one?id=${id}`)
    .then((res) => res.data)
    .catch(() => null)
}
